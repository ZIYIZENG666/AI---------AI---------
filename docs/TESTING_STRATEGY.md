...
11. Campaign status rejects `running`, `paused`, `completed`, `failed`, and
    `cancelled`; those values belong to `task_runs` or future job models.
...
## 9. Lead Discovery Contract Tests

Phase 4 Lead Discovery tests must stay limited to candidate discovery. They
must not test Lead Validation, website intelligence, AI scoring, human review,
contact discovery, Outreach Draft, or Gmail Draft behavior as if those phases
already exist.

Must test:

1. A `confirmed` Campaign can create a Lead Discovery task.
2. A `draft` Campaign cannot create a Lead Discovery task.
3. An `archived` Campaign cannot create a new Lead Discovery task.
4. Lead Discovery uses the Campaign's `product_card_snapshot`, not live Product
   Card rereads.
5. Task execution status is stored in `task_runs`, not in `campaigns.status`.
6. The task is created with `task_type = lead_discovery`,
   `related_entity_type = campaign`, and `related_entity_id = campaign_id`.
7. `pending`, `running`, and `completed` Lead Discovery tasks block a duplicate
   task for the same Campaign.
8. `failed` and `cancelled` Lead Discovery tasks may be retried with a new task
   run.
9. A successful mock search with zero results marks the task `completed`, not
   `failed`.
10. Provider or system errors mark the task `failed` and record `error_message`.
11. The first implementation uses `MockSearchProvider` and does not call a real
    search API.
12. Tests must not perform real network calls, real paid search API calls, or
    real website crawling.
13. Saved leads include `campaign_id`, `task_run_id`, `company_name`, `website`,
    `source_url`, `search_query`, `raw_snippet`, `discovery_reason`, and
    `provider_name`.
14. `website` and `source_url` are required for saved Phase 4 leads.
15. The same Campaign should not save duplicate leads with the same normalized
    website.
16. The same website may be saved under different Campaigns.
17. Saved Phase 4 leads start with `validation_status = pending` and
    `review_status = unreviewed`.
18. Saved Phase 4 leads are not eligible for scoring, approval, outreach, or
...
## 14. Database and Migration Testing
