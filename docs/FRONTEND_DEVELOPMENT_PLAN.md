...
7. Frontend Phase 5 live-backend browser smoke against local PostgreSQL:
   confirmed Campaign -> candidate Leads -> start validation -> display
   `lead_intelligence` -> repeated validation `409` ->
   insufficient-content and provider-failure states.
...
- Local PostgreSQL live-backend browser smoke passed for confirmed Campaign to
